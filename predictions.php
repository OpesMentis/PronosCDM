...
<body>
    ...
    <table width="90%" align="center" style="border-spacing: 10px;">
        ...
        <?php
        $req = $bdd->query("SELECT matchs.id AS id_match, DATE_FORMAT(date, '%d/%m, %Hh%i') AS date, date AS dt, matchs.groupe, eq1.pays AS e1, eq2.pays AS e2 FROM matchs JOIN teams eq1 ON eq1.id = matchs.team1 JOIN teams eq2 ON eq2.id = matchs.team2 WHERE date > NOW() AND played = 0 ORDER BY date ASC");
